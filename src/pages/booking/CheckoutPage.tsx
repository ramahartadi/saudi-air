...
export default function CheckoutPage() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const itemName = booking.booking_type === 'hotel' 
        ? `Hotel: ${booking.hotel_data?.name}`
      ...
      const { data, error } = await supabase.functions.invoke('midtrans-payment', {
        body: {
          ...
          params: {
            ...
            itemDetails: [
              {
                id: (booking.booking_type === 'hotel' ? booking.id : booking.flight_data?.id) || booking.id,
              ...
              <CardContent className="p-0 bg-slate-50">
                ...
                {booking.booking_type === 'hotel' ? (
                  <div className="p-6 border-b-2 border-slate-200">
                    <p className="text-[10px] font-black uppercase text-muted-foreground mb-4">Akomodasi Hotel</p>
                    <div className="flex gap-4">
                      <div className="flex-1">
                        <h4 className="font-black uppercase italic leading-tight mb-2">{booking.hotel_data?.name}</h4>
                        <div className="grid grid-cols-2 gap-2 text-[9px] font-bold">
                          <div className="bg-white border-2 border-foreground p-1 px-2 uppercase">
                            IN: {booking.hotel_data?.checkIn ? new Date(booking.hotel_data.checkIn).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' }) : '-'}
                          </div>
                          <div className="bg-white border-2 border-foreground p-1 px-2 uppercase">
                            OUT: {booking.hotel_data?.checkOut ? new Date(booking.hotel_data.checkOut).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' }) : '-'}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-4 pt-2">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-bold uppercase opacity-60">Harga ({booking.booking_type === 'hotel' ? 'Kamar' : 'Tiket'} {booking.passengers_count}x)</span>
