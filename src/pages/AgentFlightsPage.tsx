...
export default function AgentFlightsPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-black uppercase tracking-widest text-foreground">Verifying Agent Access...</p>
        </div>
      </div>
    );
  }
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (discData?.value?.agent !== undefined) {
          setDiscountRate(discData.value.agent);
        } else {
          setDiscountRate(0);
        }
  ...
  const mapSearchApiToFlight = (offer: any): Flight => {
    ...
    // AGENT DISCOUNT (Strict calculation)
          ...
          <div className="flex flex-col lg:flex-row justify-between gap-6">
            <div>
              ...
              <p className="font-bold text-emerald-600 uppercase text-xs mt-2 bg-secondary inline-block px-2 py-1 border border-foreground">
                Official Agent Portal: Exclusive {discountRate}% Discount Applied
              </p>
            </div>
          </div>
        </div>
        ...
        {loading || discountRate === null ? (
          <div className="py-20 text-center flex flex-col items-center">
            ...
            <p className="font-black uppercase tracking-widest italic">
              {discountRate === null ? 'Fetching Agent Inventory...' : 'Calculating Your Best Price...'}
            </p>
          </div>
        ) : (
