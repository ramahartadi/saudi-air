...
export default function AgentFlightsPage() {
  ...
  // Protection logic
  if (!authLoading && (role !== 'agent' && role !== 'admin')) {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        setDiscountRate(discData?.value?.agent ?? 0);
      ...
      <div className="container py-8 min-h-screen">
          ...
          <div className="mb-6 border-4 border-dashed border-rose-500 p-4 bg-rose-50 shadow-[4px_4px_0px_0px_rgba(244,63,94,1)]">
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-sm font-black uppercase text-rose-600 flex items-center gap-2">
                <ShieldCheck className="h-4 w-4" /> Admin Debug Panel (Agent Mode View)
              </h2>
              <Button 
                variant="outline" 
                size="sm" 
        ...
        <div className="bg-white border-4 border-foreground p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] mb-10">
          <div className="flex flex-col lg:flex-row justify-between gap-6">
            <div>
              ...
              <p className="font-bold text-emerald-600 uppercase text-xs mt-2 bg-secondary inline-block px-2 py-1 border border-foreground">
                Official Agent Portal: Exclusive {discountRate}% Discount Applied • {tripType.toUpperCase()}
              </p>
            </div>
          </div>
        </div>
        ...
        {loading || discountRate === null ? (
          <div className="py-20 text-center flex flex-col items-center">
            ...
            <p className="font-black uppercase tracking-widest italic">Fetching Agent Inventory...</p>
          </div>
        ) : (
          flights.length > 0 ? (
            ...
            <div className="py-20 bg-secondary border-4 border-foreground border-dashed text-center flex flex-col items-center gap-4">
              ...
              <div>
                <h3 className="text-2xl font-black uppercase italic tracking-tighter">No Agent Deals Found</h3>
